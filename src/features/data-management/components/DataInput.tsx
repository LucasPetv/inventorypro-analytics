...
interface DataInputProps {
  onProcess: (data: InventoryRow[]) => void;
  mapping: ColumnMapping;
  setMapping: (m: ColumnMapping) => void;
}

const DataInput: React.FC<DataInputProps> = ({ onProcess, mapping, setMapping }) => {
      ...
      try {
        ...
        const rows: InventoryRow[] = jsonData.slice(1).map((row: any[], index) => {
          ...
          headers.forEach((header, idx) => {
            ...
            const mappedName = Object.keys(mapping).find(key => mapping[key as keyof ColumnMapping] === header);
          ...
          const hasMappedArtikel = Object.keys(row).some(key => {
            const mappedField = Object.keys(mapping).find(field => mapping[field as keyof ColumnMapping] === key);
        ...
        if (rows.length === 0) {
          const mappedArtikelColumn = mapping.Artikel;
  ...
  const handleCSVFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      if (rows.length === 0) {
        ...
        console.log('Debug Info:', { mapping, headers, separator, firstFewRows: lines.slice(0, 3) });
        return;
      }
